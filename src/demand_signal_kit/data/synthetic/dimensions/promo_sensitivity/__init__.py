...
@register_dimension
class PromoSensitivityDimension(BaseDimension):
    dimension_name = "promo_sensitivity"
        ...
        for _, module_name, _ in pkgutil.iter_modules([str(pkg_dir)]):
            if module_name not in ("__init__",):
                importlib.import_module(
                    f"demand_signal_kit.data.synthetic.dimensions.promo_sensitivity.{module_name}"
                )
