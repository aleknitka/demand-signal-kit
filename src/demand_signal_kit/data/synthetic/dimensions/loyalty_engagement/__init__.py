...
@register_dimension
class LoyaltyEngagementDimension(BaseDimension):
    dimension_name = "loyalty_engagement"
        ...
        for _, module_name, _ in pkgutil.iter_modules([str(pkg_dir)]):
            if module_name not in ("__init__",):
                importlib.import_module(
                    f"demand_signal_kit.data.synthetic.dimensions.loyalty_engagement.{module_name}"
                )
